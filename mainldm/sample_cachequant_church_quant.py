...
if __name__ == "__main__":
    ...
    if args.ptq:
        ...
        if args.recon is False:
            pre_err_list = torch.load(f"./error_dec/church/pre_quanterr_abCov_weight{args.weight_bit}_interval{args.replicate_interval}_list.pth")
            q_unet.model.output_blocks[-1][0].skip_connection.pre_err = pre_err_list
            pre_norm_err_list = torch.load(f"./error_dec/church/pre_norm_quanterr_abCov_weight{args.weight_bit}_interval{args.replicate_interval}_list.pth")
        ...
        if args.recon:
            ...
            skip_model = skip_LDM_Model(q_unet, model_type="church")
    ...
    imglogdir = "./error_dec/church/image"
